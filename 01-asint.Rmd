...
__Ejercicio 2:__ Compruebe, en función de lo anterior, que la distribución de Cauchy tiene dominio de atracción máximo (DAM) de la distribución de Fréchet (Solución en el Apéndice \@ref(sec:ejercicios)).
...
Observar que con el cambio de variable $u=M_F-1/x$, resulta que $1-F(u)=(^{-}M_F-u)^{\alpha} L(1/(M_F-u)) para
...
::: {.theorem #th101 name="Dominio de Atracción Máxima (DAM) de la Weibull"}

**a.** Sea \( F \) una función de distribución con extremo superior finito \( M_F := \sup\{x : F(x) < 1\} < \infty \).  
Entonces \( F \) pertenece al dominio de atracción máxima de la distribución de **Weibull**, denotada \( \Psi_{\alpha} \), si y solo si existe una función de **variación lenta** \( L \in \mathcal{R}_0 \) tal que:

\[
1 - F\left(M_F - \frac{1}{x} \right) = x^{-\alpha} L(x), \quad \text{para } x \to \infty.
\]

Es decir, la cola izquierda de la distribución decrece como una **potencia del gap hasta \( M_F \)**.

Con el cambio de variable \( u = M_F - \frac{1}{x} \), esta condición puede escribirse también como:

\[
1 - F(u) = (M_F - u)^{\alpha} \cdot L\left( \frac{1}{M_F - u} \right), \quad u < M_F,
\]

donde \( L \) sigue siendo una función de variación lenta.

Un ejemplo típico es:

\[
1 - F(u) = (M_F - u)^{\alpha}, \quad u < M_F,
\]

que corresponde al caso en que \( L(x) \equiv 1 \).

En este caso, se puede tomar como normalización:
- \( d_n = M_F \),
- \( c_n = M_F - U(n) \),  
donde \( U(n) = F^{\leftarrow}(1 - 1/n) \) es la **cuantil de orden extremo**, y se cumple:

\[
\lim_{n \to \infty} F^n(d_n - c_n x) = \Psi_{\alpha}(x) := \exp\left( -x^{\alpha} \right), \quad x > 0.
\]

---

**b.** (*Condición de Von Mises II*)  
Si \( F \) es una distribución con densidad \( f \) positiva en un intervalo \( (a, M_F) \), para algún \( a < M_F \), y además se cumple que:

\[
\lim_{x \to M_F^-} \frac{(M_F - x) f(x)}{1 - F(x)} = \alpha,
\]

entonces \( F \in DAM(\Psi_{\alpha}) \).  
Esta condición garantiza que la función de distribución se comporta como una potencia cerca del extremo superior, y por tanto pertenece al dominio de atracción de Weibull.

Más aún, **toda distribución cuya cola izquierda sea equivalente a una que cumpla esta condición también pertenece al \( DAM(\Psi_{\alpha}) \)**.
