...
El comienzo del curso se centra en la teoría más clásica de estadística de datos extremos, basada en los trabajos de @frechet1927, @gumbel1958, @weibull1951, @fisher1928limiting, @gnedenko1943, entre otros. En este marco, los argumentos que manejamos son de tipo __asintóticos__^[El análisis asintótico es un método de descripción del comportamiento en el límite.] y podríamos decir que nos ubicamos en lo que se conoce en la literatura como _paradigma de los valores extremos_ [@coles2001introduction].

En la teoría de valores extremos^[EVT por sus siglas en en inglés relativas a Extreme Value
Theory.], existen dos enfoques fundamentales, ambos ampliamente utilizados: el método de máximos por bloques (BM)^[BM, por sus siglas en inglés relativas a Block Maxima.] y el método de excedencias sobre umbral (POT)^[POT, por sus siglas en inglés.] [@ferreira2015]. En este capítulo se pone el foco en el método BM y más adelante veremos el POT. 

La metodología de BM en la EVT consiste en dividir el período de observación en bloques no superpuestos de igual tamaño y considerar únicamente el valor máximo observado en cada bloque. Las nuevas observaciones así generadas seguirán (bajo ciertas condiciones) alguna de las distribuciones que veremos en este capítulo (@ferreira2015, gumbel1958). 




__Observación 1__ (Distribución del máximo de dos variables independientes).
Se recuerda que si \( X \) e \( Y \) son variables aleatorias __independientes__, con funciones de distribución acumulada (cdf) \( F \) y \( G \), respectivamente, entonces, la función de distribución acumulada (cdf) de la variable aleatoria $Z = \max(X, Y)$ es
...
\begin{equation}
H(t) = P(\max(X, Y) \le t) = P(X \le t, Y \le t) = F(t) \cdot G(t)
(\#eq:HZ)
\end{equation}
...
\begin{equation}
P(X \le t, Y \le t) = P(X \le t) \cdot P(Y \le t).
(\#eq:indep)
\end{equation}


__Observación 2__ (Datos independientes e idénticamente distribuidos ($iid$)).  En esta parte inicial del curso
...
__Observación 3.__ Resulta de la Observación 1, que si tenemos la secuencia de datos $X_1,\dots,X_n$, que son $iid$ con función de distribución conjunta $F$, entonces
...
<!--
\begin{equation}
M_n= \max \left( X_1,\dots,X_n \right)\quad\text{tiene distribución}\quad F^{n}.
\end{equation}
...
En teoría, la distribución de $M_n$ se podría derivar a partir de todos los valores de $n$ tal que 

\begin{align}
P\left( M_n \leq z\right)&=P\left( X_1 \leq z,\dots,X_n \leq z\right)\nonumber\\
&=P\left( X_1 \leq z\right)\times \dots \times P\left(X_n \leq z\right) \nonumber \\
&=\left\{ F(z) \right\}^n 
(\#eq:Mn)
\end{align}

donde $z \in Soporte(F)$ y \( F(z) = P(X_i \leq z) \) es la función de distribución de cada variable \( X_i \). Si conocieramos la distribución $F$ conoceríamos la distribución $F^{n}$, pero en algunos casos la lectura que queda registrada es la del dato máximo, y no la de cada observación que dio lugar al mismo, por lo que a veces ni siquiera es viable estimar $F$ [@coles2001introduction]. Pero aún en los casos en que $F$ es conocida o estimable, si $n$ es grande, la fórmula de $F^{n}$ puede resultar prácticamente inmanejable. En una línea de trabajo similar a la que aporta el Teorema Central del Límite (TCL) [@bertsekas2008introduction] en la estadística de valores medios (ver Apéndice  \@ref(sec:ap)), vamos a ver un teorema que nos va a permitir aproximar $F^{n}$ por distribuciones más sencillas. Este es el Teorema de Fischer-Tippet-Gnedenko (FTG) que presentaremos en breve (@fisher1928limiting,  @gnedenko1943).



__Observación 4.__ Si la secuencia de datos $X_1,\dots,X_n\;$ es $iid\;$ y definimos
$\;Y_i = -X_i\;$ para todo valor de $i$, entonces $Y_1,\dots,Y_n\;$ es $iid\;$ y además
...
\begin{equation}
M_t=\max\left\{ R_1, \dots, R_t\right\}
\end{equation}

es el máximo rendimiento diario observado en un período de $t$ observaciones.
...
donde \( F(z) = P(X_i \leq z) \) es la función de distribución de cada variable \( X_i \). 
-->

donde $X_1, \dots, X_n$ es la secuencia de variables aleatorias independientes que tienen una distribución común $F$. Como ya mencionamos, en teoría, la distribución de \( M_n \) podría derivarse exactamente para todos los valores de \( n \) tal que $P\left( M_n \leq z\right)=\left\{ F(z) \right\}^n$ como se expresa en la Ecuación \@ref(eq:Mn). Sin embargo, en la práctica esto no es útil porque la función de distribución $F$ es desconocida. Por este motivo, una solución consiste en aproximar _familias de modelos_ para $F^n$ que se estiman únicamente en base a datos extremos:

- Vamos a emplear argumentos análogos a los del TCL: estudiamos el comportamiento de $F^n$ cuando $n\rightarrow \infty$. 

- Lo anterior por sí solo no es suficiente: para cualquier \( z < z^+ \), donde \( z^+ \) es el extremo superior^[Sea \( z^+ \) el menor valor de \( z \) tal que \( F(z) = 1 \). O sea, es el valor más bajo a partir del cual la función de distribución ya no crece más. Es el "techo" natural de la variable aleatoria.] de la función de distribución \( F \), se cumple que \( F^n (z) \to 0 \) cuando \( n \to \infty \), lo que implica que la distribución de \( M_n \) degenera en una masa puntual en \( z^+ \).
...
```{r colas, fig.cap="CDF extremales" , echo=FALSE, fig.height=3, fig.width=6, message=FALSE, warning=FALSE}
...
En la distribución Fréchet, cuanto menor es el parámetro de forma \( \alpha \), más pesada es la cola y más lenta la convergencia a 1 (Figura \@ref(fig:frcolas)). En cambio, la Weibull tiene soporte acotado superior y el parámetro \( \alpha \) afecta principalmente la forma de la cola izquierda^[Porque la Weibull tiene soporte en $(-\infty, 0]$.], hacia \( -\infty \) (Figura \@ref(fig:weicolas)). 

```{r frcolas, fig.cap="Distribución Fréchet para distintos parámetros de forma", echo=FALSE, fig.height=3, fig.width=6, message=FALSE, warning=FALSE}
...
```{r weicolas, fig.cap="Distribución Weibull para dististos parámetros de forma", echo=FALSE, fig.height=3, fig.width=6, message=FALSE, warning=FALSE}
# Dominio negativo
...
```{r distributions, fig.cap="PDF extremales (alpha=1)" , echo=FALSE, fig.height=3, fig.width=6, message=FALSE, warning=FALSE}
...
__Nota:__ En otros contextos de la Estadística (en particular en algunas rutinas de `R`), se le llama Weibull a una variable que corresponde a $-X$, con $X$ Weibull como definimos nosotros.

__Observación 5.__  La función Gamma permite expresar la media, la varianza y otras propiedades estadísticas de la GEV (ver Apéndice \@ref(sec:gev)) en función de su parámetro de forma $\xi$. Recordamos que la función Gamma ($\Gamma$ ), que extiende a la función factorial
($\;\Gamma(n)=n-1!\quad \forall n\;$ natural) está definida por
...
\vspace{0.3cm}  
__Parte 1)__ Si $X$ tiene distribución Gumbel $\Lambda^{(\mu,\beta)}$ entonces tiene: 
...
\vspace{0.5cm}
__Parte 2)__ Si $X$ tiene distribución Weibull $\Psi^{(\mu, \beta)}$ entonces tiene: 
...
__Parte 3)__ Si $X$ tiene distribución Fréchet $\Phi_{\alpha}^{(\mu, \beta)}$ entonces tiene: 
...
llamamos $F^n$ a la distribución de $max(X_1,\dots,X_n)$. Si $n$
es grande, entonces existen $\mu$ real y $\beta > 0$ tales que
alguna de las siguientes tres afirmaciones es
correcta:

a. $F^n$ se puede aproximar por la distribución de $\mu+\beta Y$, con $Y$ variable con distribución $\Lambda$.

\vspace{1cm}

b. Existe $\alpha>0$ tal que $F^n$ se puede aproximar por la distribución de $\mu+\beta Y$ con $Y$ variable con distribución $\Phi_{\alpha}$. 

\vspace{1cm}

c. Existe $\alpha>0$ tal que $F^n$ se puede aproximar por la distribución de $\mu+\beta Y$ con $Y$ variable con distribución $\Psi_{\alpha}$. 
:::
...
- Si $F$ es normal o exponencial, se aplica a $F^n$ la aproximación por una Gumbel .
- Si $F$ es uniforme, vale para $F^n$ la aproximación por una Weibull.
- Si $F$ es Cauchy, la aproximación válida para $F^n$ es por una Fréchet.
...
Por consiguiente el Teorema \@ref(thm:FTG) alienta a intentar ajustar datos extremales a una de las tres distribuciones extremales, pero no siempre un tal ajuste dará un resultado afirmativo.
...
O3<-c(430.3, 115.7,4.48, 26.95, 72.27,206.4, 22.79,25.03,226.8,
      11.1,1572,100,104.5,37.1,20.22,106.9,
      47.2,62.82,39.3,18.52,41.57,429.5,1228,
      127.6,9.93,90.4,201.7,295.1,20.62,20.58,
      538.1,804,321.6,16.11,22.05,100.2,40.76,262.7,
      19.32,7.79,58.02,28.02,
      18.38,13.12,572.8,44.46,
      40.72,25.07,24.07,511.8,38.12,15.86,75.84,
      24.09,119.4,174.7,104.7,140,
      79.67,158,25.46,462.5,35.53,876.4,
      462.5,53.47,23.59,38.77,494.2,164.2,
      52.06,54.13,15.53,29,14.35,1675,15.01,72.07,22.99)
...
El __objetivo__ del estudio en esta etapa es conocer la distribución de estos datos y en particular estimar la probabilidad de que el máximo estival en los 80 puntos supere el valor 50 (correspondiente a existencia de riesgo moderado).


```{r parismax, fig.cap="Máximos diarios de O3 en la región Parisina" , echo=FALSE, fig.height=3, fig.width=6, message=FALSE, warning=FALSE}

plot(O3)
```


En la Figura \@ref(fig:parismax), vemos que hay algunos datos de máximos que se disparan, brindando pistas claras sobre la DEA candidata. 



__Supuesto de datos $iid$:__  Como la mayoría de tests de ajustes suponen datos $iid$, realizaremos dos tests de aleatoriedad^[En inglés es _randomness_.]: 

a. Runs test (Up & Down) : detecta si la dirección de cambio (sube/baja) ocurre al azar o en episodios persistentes ([runs tests en R](https://www.geeksforgeeks.org/r-machine-learning/how-to-perform-runs-test-in-r/)).

b. Spearman correlation of ranks : contrasta relación monótona; en series, suele usarse contra el tiempo (tendencia) o dependencia serial monótona ([Spearman correlation of ranks en R](https://www.geeksforgeeks.org/r-language/spearman-correlation-testing-in-r-programming/)).


__Runs test (Up & Down):__ 

Aplicamos la prueba de rachas con la función run.tests del paquete `tseries`.
...
Esta prueba busca aleatoriedad en la serie de datos observada examinando la frecuencia de rachas. Una _racha_ se define como una secuencia consecutiva de respuestas similares.
...
Considerando un nivel de confianza del $95\%$, o sea, para un nivel de significación de $\alpha=0.05$, se tiene que el $p-valor > \alpha=0.05$. Con este resultado no se rechaza $H_0$, la secuencia de subidas y bajadas es compatible con aleatoriedad; no hay evidencia de patrón sistemático en la dirección de los cambios.
Vemos que no rechazamos la hipótesis nula de rachas. Continuando con la idea de ajustar una distribución extremal

__Spearman correlation of ranks:__

Vamos a analizar: Spearman vs tiempo (tendencia monótona)
...
Conclusiones según los resultados que se obtengan: 

- Si $\rho > 0$ y $p\text{-valor} < 0.05 \Rightarrow$ tendencia monótona creciente.
- Si $\rho < 0$ y $p\text{-valor} < 0.05 \Rightarrow$ tendencia monótona decreciente.
- Si $p\text{-valor} \ge 0.05 \Rightarrow$ sin evidencia de relación monótona.
...
Dado que visualmente (Figura \@ref(fig:parismax)) se aprecian valores muy apartados, se
presume una distribución de colas pesadas y por
ese motivo se intenta un ajuste a una Fréchet.



¿Qué tipo de distribución extremal le parece que capture esta dinámica de máximos? ¿como se refleja en el indice de una GEV?
...
La estimación de `shape` un número positivo, por lo que estamos ante posibles colas pesadas (como se intuia de ver la figura de los valores). Se estimó una Fréchet pues $shape>0$.





Calculamos la probabilidad de excedencia para el valor 50:
...
p_exc_mod <- pgev(50, loc = par["location"], 
                  scale = par["scale"], 
                  shape = par["shape"],
                  lower.tail = FALSE)   
...
p_exc_fr <- 1 - pfrechet(50, loc = muF, 
                         scale = betaF, shape = alpha)
as.numeric(p_exc_fr)
```
...
__Test de ajuste $\chi^2$:__ Para realizar el ajuste utilizaremos un conocido test de ajuste de carácter general: el test $\chi^2$  de ajuste. Este test requiere elegir una partición mas o menos arbitraria de la recta real en intervalos. Sin embargo, es importante que en cada intervalo caiga una cantidad suficiente de datos de la muestra; en este caso hemos tomado como extremos de los intervalos los quintiles empíricos de nuestra muestra. Una aclaración mucho más importante es que este test requiere estimar parámetros por el método de Máxima Verosimilitud Categórica (@graneri2002pearsonian), que da resultado distintos al método de Máxima Verosimilitud a secas. Este hecho es frecuentemente ignorado y presentado erróneamente en los textos y cursos básicos de Estadística.
\newpage
...
observed <- hist(O3, breaks = breaks, 
                 plot = FALSE)$counts 
```
...
```{r echo=FALSE}
fit_cat$par  # parámetros estimados por MVC
```
...
- Si el $p\text{-valor} < \alpha_{\text{signif}}$, se rechaza $H_0$ $\Rightarrow$ el ajuste no es bueno.
- Si el $p\text{-valor} \ge \alpha_{\text{signif}}$, no hay evidencia para rechazar $H_0$ $\Rightarrow$ el modelo es compatible con los datos.
...
```{r parismasx, fig.cap="Máximos diarios durante el verano", fig.height=3, fig.width=6, echo=FALSE, message=FALSE, warning=FALSE}
...
- Runs test (Up & Down) : [runs tests en R](https://www.geeksforgeeks.org/r-machine-learning/how-to-perform-runs-test-in-r/) 
...
```{r parishist, echo=FALSE, fig.cap="Histograma de los máximos diarios", fig.height=3, fig.width=6, message=FALSE, warning=FALSE}
...
__Observación 13.__ El resultado anterior vale incluso si $M_F$ es infinito, pero si $M_F$ es finito, como
...
__Observación 14.__ Si $F$ es $Bin(m,p) \Rightarrow M_F=m$. Si $X$ tiene distribución $F$, entonces
...
__Observación 15.__ Si $F$ es una distribución de Poisson de parámetro $\lambda >0$, $M_F$ es infinito. Si $k$ es un natural, entonces
...
__Observación 16.__ El Teorema \@ref(thm:th6) brinda una condición __necesaria__ pero __no suficiente__
...
__Observación 17.__  Si $F$ es $U(0,1)$ y consideramos
...
__Observación 18.__ Si $F$ es Exponencial de
...
__Observación 19.__ Si $F$ es $N(0,1)$ y consideramos
...
__Observación 20.__ Si $F$ es Cauchy standard que se expresa como $C(0,1)$ 
...
__Observación 21.__ Si $F$ y $G$ son dos distribuciones, van a tener colas equivalentes si $M_F=M_G$, y cuando $t$
...
__Observación 22.__  Se dice que una distribución \( F \) tiene cola de __variación regular__ de orden \( -\alpha \) (con \( \alpha \geq 0 \)) si, para todo \( t > 0 \),
\[
...
__Observación 23.__ En algunas situaciones los datos extremales pueden ajustarse a más de un modelo. Por ejemplo, puede ocurrir que tanto ajusten los datos una Gumbel como una Weibull. Frente a estas situaciones, no hay una receta única de cómo proceder sino que quien está modelando debe tener claro si corresponde volcarse hacia cálculos más pesimistas (que dan mayor probabilidad a eventos extremos muy severos) o más optimistas. 
...
__Observación 24.__ Antes de seguir adelante, demos la respuesta a la parte a) del Ejercicio 5. Es un
...
::: {.example name="Máximos de abundancia poblacional de fitoplancton"}
Una aplicación relevante del enfoque de valores extremos generalizados (GEV) se presenta en @segura2023metabolic, donde se modelan máximos de abundancia poblacional de fitoplancton como función del tamaño corporal y recursos ambientales. El ajuste de modelos GEV por clases de tamaño permitió evidenciar escalamiento negativo de la abundancia máxima con el tamaño corporal, en línea con predicciones teóricas basadas en restricciones metabólicas.

Además, se modelaron los parámetros de la distribución GEV como funciones de temperatura y nutrientes, permitiendo capturar la variabilidad residual con mayor precisión que usando modelos gaussianos. Este enfoque ilustra cómo incorporar covariables ecológicas relevantes en el modelado de extremos y cómo evitar sesgos importantes en la estimación de tiempos de retorno.
:::
...
Vamos a usar el paquete `evd` de `R` y de los datos `uccle` tomamos la columna `day` que contiene el máximo anual de precipitaciones diarias. Es decir, para cada año, se tomó el día con la lluvia más alta registrada, y ese valor es el que se guarda en la columna `day`.
...
En la siguiente tabla se detalla la evolución del valor de retorno $z_T$ a largo plazo según cada una de las distribuciones extremales ya conocidas.
...
\begin{table}[ht]
\centering
\caption{ Características de los tres tipos de la distribución~GEV}
\begin{tabular}{|l|c|l|p{7cm}|}
\hline
\textbf{Tipo de GEV} & $\boldsymbol{\xi}$ & \textbf{Forma} & \textbf{Implicación sobre el valor de retorno $z_T$ a largo plazo} \\
\hline
\textbf{Weibull}  & $\xi < 0$ & se aplana (convexa) & Tiene un \textbf{máximo}: no importa cuánto esperes, no pasa de ahí \\
\hline
\textbf{Gumbel}   & $0$       & lineal en $\log T$  & Crece lentamente \\
\hline
\textbf{Fréchet}  & $\xi > 0$ & cóncava             & Crece \textbf{sin límite}: cuanto mayor el $T$, \textbf{más extremo} es el valor esperado \\
\hline
\end{tabular}
 (\#tab:vr)
\end{table}
...
# Filtrar datos en el rango 0–100
sel <- u_vals >= 0 & u_vals <= 100

plot(u_vals[sel], p_gumbel[sel], type = "l", col = "blue", lwd = 2,
     ylim = c(0, 1), xlab = "u", ylab = expression(P(X > u)), 
     main = "(a) Probabilidades de Excedencia",
     cex.lab = 2.5, cex.axis = 2, cex.main = 3)

lines(u_vals[sel], p_frechet[sel], col = "red", lwd = 2, lty = 2)

legend("topleft", legend = c("Gumbel", "Fréchet"), 
       col = c("blue", "red"), lty = 1:2, lwd = 2, bty = "n", cex = 2.5)
...
Con los datos del Ej. \@ref(exm:mv), vamos ahora a calcular los valores de retorno (VR) y los tiempos de retorno (TR).
:::

Para calcular los valores de retorno, primero definimos un vector con los periodos de retorno (en años) para los que queremos estimar dichos niveles. Por ejemplo: 2, 5, 10, 15, 25, 50, 100, 150 y 200 años.

```{r echo=TRUE}
# Años para los que vamos a calcular los valores de retorno
tpo <- c(2, 5, 10, 15, 25, 50, 100, 150, 200)
```

A partir de estos valores, calculamos las probabilidades asociadas:

```{r echo=TRUE}
# Probabilidades asociadas a los periodos de retorno
prob <- 1 / tpo
```

Utilizamos los parámetros estimados para la distribución de Gumbel ajustada:

```{r echo=TRUE}
# Parámetros del ajuste Gumbel: loc, scale
z <- fit_gumbel$estimate
```

Y finalmente, calculamos los valores de retorno mediante la función cuantílica `qgumbel`:

```{r echo=TRUE}
# Cálculo directo usando lower.tail = FALSE
round(qgumbel(prob, z[1], z[2], lower.tail = FALSE), 1)
```

```{r echo=TRUE}
# Alternativamente, usando 1 - prob
round(qgumbel(1 - prob, z[1], z[2]), 1)
```

Obtenemos así los valores máximos diarios esperados para cada uno de los periodos de retorno definidos.

\vspace{1cm}

Queremos ahora calcular el TR para una precipitación máxima concreta:

```{r echo=TRUE}
# Calcular el valor máximo observado
maximo_obs <- max(datos)
round(maximo_obs,0)
```


```{r echo=TRUE}
# Extraer parámetros estimados del ajuste Gumbel
z <- fit_gumbel$estimate  # z[1] = loc, z[2] = scale

# Calcular el periodo de retorno correspondiente a ese valor máximo
periodo_retorno <- 1 / (1 - pgumbel(maximo_obs, z[1], z[2]))

# Mostrar el resultado redondeado
round(periodo_retorno, 0)
```

O sea que, podríamos esperar una precipitación máxima diaria de 72 mm, por término medio, cada 68 años.
