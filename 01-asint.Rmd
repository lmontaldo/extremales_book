...
__Nota:__ En otros contextos de la Estadística (en particular enalguna rutinas de R), se le llama Weibull a una variable que corresponde a $-X$, con $X$ Weibull como definimos nosotros.
...
\vspace{0.3cm}  
__Parte 1)__ Si $X$ tiene distribución $\Lambda^{(\mu,\beta)}$ entonces tiene: 
...
\vspace{0.5cm}
__Parte 2)__ Si $X$ tiene distribución $\Psi^{(\mu, \beta)}$ entonces tiene: 
...
__Parte 3)__ Si $X$ tiene distribución $\Phi_{\alpha}^{(\mu, \beta)}$ entonces tiene: 
...
c. Existe $\alpha>0$ tal que $F^n$ se puede aproximar por la distribución de $\mu+\beta Y$ con $Y$ variable con distribución $\Phi_{\alpha}$. 
:::
