...
Theory.), existen dos enfoques fundamentales, ambos ampliamente utilizados: el método de máximos por bloques (BM)^[BM, por sus siglas en inglés relativas a Block Maxima.] y el método de excedencias sobre umbral (POT)^[POT, por sus siglas en inglés.] [@ferreira2015]. En este capítulo se pone el foco en el método por BM y más adelante veremos el POT. 




The block maxima (BM) approach in extreme value
theory (EVT), consists of dividing the observation period into nonoverlapping periods of equal size and restricts attention to the maximum observation in each period
